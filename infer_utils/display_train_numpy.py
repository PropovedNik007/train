...
class display_train(object):
    ...
    def get_norm_box1(self, boxes, class_names):
        boxes = boxes[::-1]
    ...
    def get_norm_box(self, boxes, class_names):
        ...
        for i in range(0, len(boxes)):
            ...
            y2 = bottom_list[i]
            for box in boxes[i]:
                res_box = [box[0].item(), y1, box[2].item(), y2, box[5].item()]
                norm_boxes.append(res_box)
    ...
    @staticmethod
    def class_filter_where(norm_boxes, class_rails):
        for i, boxes in enumerate(norm_boxes):
            for j, box in enumerate(boxes):
                np.where((norm_boxes[i][1] == norm_boxes[j][1]), norm_boxes, 0)
        return norm_boxes[np.where(norm_boxes == class_rails)]
    ...
    def init_current_rails(self, norm_boxes, class_rails, output_image):
        ...
        filter_unique_central_rails = self.class_filter_where(norm_boxes, class_rails)
        # print(filter_unique_central_rails)
        current_box = []
        for i, box in enumerate(filter_unique_central_rails):
            overlap = area(filter_unique_central_rails[i], filter_unique_central_rails[i - 1])
